...
from utils.utils import ACCEPTED_WSI_TYPES, decode_geojson_to_mask, divide_list_slurm_array, get_file_list, save_geojson_annotation, save_sparse_annotation, get_slide_level_result, combine_results
...
def merge_mucin_and_ln(ln_seg_file_all:np.ndarray, ln_class:int, mucin_class:int) -> np.ndarray:
    ...
    if mucin_class not in ln_seg_file_all:
        return ln_seg_file_all, ln_mask
...
def main(args):
    ...
    if args.prepare_wsi_level_result_csv:
        ...
        if (os.path.exists(finished_csv_path) and slurm_id != int(os.environ.get('SLURM_ARRAY_TASK_COUNT', 1)) - 1) or os.path.exists(global_csv_path):
            # if the finished file exists, we can skip this task
            print(f"Finished file {finished_csv_path} already exists, skipping this task")
            sys.exit(0)
    ...
    with tqdm(total=len(wsi_files)) as pbar:
        for wsi_file in wsi_files:
            ...
            # if args.mucin_class in ln_seg_file_all:
            #     ln_seg_file = post_process(segmentation_mask=ln_seg_file_all.copy(),
            #                                     lymph_node_class=args.ln_class,
            #                                     classes_to_merge=[args.mucin_class],
            #                                     merge_thresholds=[0.05],
            #                                     erase_thresholds=[0.01],
            #                                     apply_opening=[False],
            #                                     min_ln_area=0)
            #     ln_seg_file = (ln_seg_file == args.ln_class).astype(np.uint8)
            #     mucin_outside_ln = ((ln_seg_file_all == args.mucin_class) & (ln_seg_file!=1))
            #     ln_seg_file_all[mucin_outside_ln] = 0
            #     del mucin_outside_ln
            # else:
            #     ln_seg_file = (ln_seg_file_all == args.ln_class).astype(np.uint8)
                pred_mask, level, level_downsampling, exact_resolution, read_origin = infer_wsi(model, wsi_file, ln_seg_file, args.batch_size, args.tile_size, args.step_size, args.crop_pred_edge, args.resolution, downsample_factor)
            ...
            print(f'Processed {wsi_name}: {measurement:.2f} um - {status} - {label}')
