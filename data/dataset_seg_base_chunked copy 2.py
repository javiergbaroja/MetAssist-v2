...
[B8]  Fix tuple-multiply bug in __getitem__  (correctness)
      `tile_size_wh * self.tiling_downsample_factor` where tile_size_wh is a
      2-tuple yields a 4-tuple for factor > 1  ((w,h)*2 = (w,h,w,h)).
      Fixed by explicit scalar multiplication of each dimension.

BaseTileDataset
---------------
Both definitions kept unchanged from original.
"""
...
# ---------------------------------------------------------------------------
# _stitch_meta element layout (one plain tuple per tile)
# ---------------------------------------------------------------------------
#
# (rs, re, cs, ce,           prediction-space crop window
#  t_row_s, t_row_e,         row slice into raw pred_logits (before crop removal)
#  t_col_s, t_col_e,         col slice into raw pred_logits
...
class BaseSlideDataset:
    ...
    GPU-resident accumulation buffers, fork-safe OpenSlide handles,
    and fully pre-computed stitch metadata.  See module docstring.
    """
    ...
    def __init__(
        ...
        # Global tile index — maps stitch_predictions call to _stitch_meta entries
        self._tile_idx = 0
    ...
    def wsi(self) -> openslide.OpenSlide:
        ...
        The init-time handle is closed at the end of __init__, so self._wsi
        is None when DataLoader workers are forked.  Each worker (and the main
        process for num_workers=0) transparently reopens on first __getitem__.
        This is the only change needed to make openslide fork-safe.
        """
    ...
    def _prepare_slide(self, wsi_path: str, resolution: float) -> None:
        """
        Resolve local path (TMPDIR copy if applicable), open a temporary handle
        in self._wsi, and populate all WSI geometry attributes.

        The handle must remain open until coord extraction completes in __init__.
        __init__ closes it immediately after.
        """
        ...
        # [B3] Persist resolved path for lazy reopening
        self._wsi_path = local_wsi_path
        # Open temporary init-time handle
    ...
    # ------------------------------------------------------------------
    # Chunk assignment — with pre-computation  [B6]
    # ------------------------------------------------------------------
    ...
    def _cropped_pred_coords(
        ...
        """
        Convert raw tile origin to prediction-space crop window.
        Called only during _assign_tiles_to_chunks (init); results cached.
        """
    ...
    def _assign_tiles_to_chunks(self) -> None:
        ...
        Replaces self._tile_to_chunks.  Every intersection slice is computed once
        here; stitch_predictions does no arithmetic at all.
        ...
        chunk_data element:
            (chunk_id, b_rs, b_re, b_cs, b_ce, lt_rs, lt_re, lt_cs, lt_ce)
        """
        for coord in self.coords:
            ...
            for cr in range(cr_start, cr_end + 1):
                for cc in range(cc_start, cc_end + 1):
                    ...
                    # Intersection in prediction space
    ...
    def _allocate_chunk(self, chunk_id: Tuple[int, int]) -> None:
        ...
        GPU: FP16  (5-class 4096x4096 chunk = 160 MB, fits easily in 24 GB VRAM)
        CPU: FP32  (safer for long accumulation chains)
        """
        cr, cc = chunk_id
    ...
    def _finalize_chunk(self, chunk_id: Tuple[int, int]) -> None:
        """
        [B5] Average logits, argmax on _stitch_device, write uint8 to output.

        Only the uint8 argmax is transferred CPU-side — 8x less data than
        transferring the full FP logit volume.
        """
        ...
        # argmax on device, then cheap uint8 D2H (or noop if already CPU)
    ...
    def stitch_predictions(
        ...
        tile_predictions: (B, C, H, W) — may be on any device / dtype.
            Moved to self._stitch_device + buffer dtype before accumulation.
            When the caller already passes a tensor on the correct device
            (e.g. infer_wsi with GPU stitch), no transfer occurs.

        [B6] Zero arithmetic per tile: all slice indices are read directly from
        self._stitch_meta (precomputed at init).
        """
        if self._num_classes is None:
            self._num_classes = tile_predictions.shape[1]

        # Ensure tensor lives on stitch device with correct dtype
        buf_dtype = (
            torch.float16 if self._stitch_device.type == "cuda" else torch.float32
        )
        ...
        for pred_logits, augmentation in zip(tile_predictions, augmentations):
            ...
            # --- 1. Inverse TTA (no-op for apply_tta=False) ---
            ...
            # Crop tile logits to the contribution window
            cropped = pred_logits[:, t_row_s:t_row_e, t_col_s:t_col_e]

            # --- 3. Accumulate into each overlapping chunk ---
            for (chunk_id,
                 b_rs, b_re, b_cs, b_ce,
                ...
                # --- 4. Finalise when all contributing tiles are done ---
                self._chunk_remaining[chunk_id] -= 1
    ...
    def create_final_predictions(
        ...
        Any chunks not finalised during stitch_predictions (e.g. interrupted
        inference) are resolved here as a safety net.
        """
        if return_probs:
            raise ValueError(
                "return_probs=True is not supported with chunked stitching: "
                "chunk buffers are freed incrementally during stitch_predictions. "
                "Set chunk_size to cover the entire WSI or accumulate externally."
            )
    ...
    def __getitem__(
        ...
        # [B8] Fix: scalar-multiply each dimension separately.
        # Original `tile_size_wh * factor` where tile_size_wh is a 2-tuple
        # yields a 4-tuple for factor > 1: (w, h) * 2 = (w, h, w, h).
